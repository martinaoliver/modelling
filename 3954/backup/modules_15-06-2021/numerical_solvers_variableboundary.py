...
def adi_ca(par_dict, initial_condition, L_x, L_y, J, I, T, N, circuit_n, boundary_coef=1, perturbation=0.001,timeresolution='fast',n_species = 6,tqdm_disable=True,p_division=1):
    ...
    for n in tqdm(range(N),disable = tqdm_disable):
        ...
        if n_species==6:
            ...
            for i in range(I):
                ...
                C_new[i, :] = (solve_banded((1, 1), matrix_A_list[2], b_X(2, i) + f_curr_C[i, :] * dt / 2))
                D_new[i, :] = (solve_banded((1, 1), matrix_A_list[3], b_X(3, i) + f_curr_D[i, :] * dt / 2))
                E_new[i, :] = (solve_banded((1, 1), matrix_A_list[4], b_X(4, i) + f_curr_E[i, :] * dt / 2))
                F_new[i, :] = (solve_banded((1, 1), matrix_A_list[5], b_X(5, i) + f_curr_F[i, :] * dt / 2))
            ...
            for j in range(J):
                ...
                C_new[:, j] = (solve_banded((1, 1), matrix_C_list[2], d_X(2, j) + f_curr_C[:, j] * dt / 2))
                D_new[:, j] = (solve_banded((1, 1), matrix_C_list[3], d_X(3, j) + f_curr_D[:, j] * dt / 2))
                E_new[:, j] = (solve_banded((1, 1), matrix_C_list[4], d_X(4, j) + f_curr_E[:, j] * dt / 2))
                F_new[:, j] = (solve_banded((1, 1), matrix_C_list[5], d_X(5, j) + f_curr_F[:, j] * dt / 2))
