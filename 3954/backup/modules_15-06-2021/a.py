...
def adi_shape(par_dict, steadystates, L_x, L_y, J, I, T, N, circuit_n, shape_name, boundary_coef=1, perturbation=0.001,timeresolution='fast',n_species = 6,tqdm_disable=True):
...
def adi_shape_ca(par_dict, steadystates, L_x, L_y, J, I, T, N, circuit_n, shape_name, boundary_coef=1, perturbation=0.001,timeresolution='fast',n_species = 6,tqdm_disable=True,p_division=1):
...
#1D PLOTTING
# def plot_1D_final_concentration(final_concentration,grids,mechanism,shape,filename,parID,save_figure=False,path=''):
#
#     A,B,C,D,E,F = final_concentration
#     x_grid = grids[0]
#
#
#     fig, (ax1,ax2,ax3,ax4,ax5,ax6) = plt.subplots(6, figsize=(4,4))
#     # fig.suptitle('parID%r'%parID,fontsize=5)
#
#
#     ax1.plot(x_grid, A, 'k', label = 'A')
#     ax1.set( ylabel = '[A]', ylim=(0, round(np.amax(A), 3) + 0.15*np.amax(A)))
#     ax1.set_xticklabels('')
#
#     ax2.plot(x_grid, B,'k', label = 'B')
#     ax2.set(  ylabel = '[B]', ylim=(0, round(np.amax(B), 3) + 0.15*np.amax(B)))
#     ax2.set_xticklabels('')
#
#     ax3.plot(x_grid, C, 'k',label = 'C')
#     ax3.set( ylabel = '[C]', ylim=(0, round(np.amax(C), 3) + 0.15*np.amax(C)))
#     ax3.set_xticklabels('')
#
#     ax4.plot(x_grid, D, 'g', label = 'D')
#     ax4.set( ylabel = '[D]', ylim=(0, round(np.amax(D), 3) + 0.15*np.amax(D)))
#     ax4.set_xticklabels('')
#
#     ax5.plot(x_grid, E,'r', label = 'E')
#     ax5.set( ylabel = '[E]', ylim=(0, round(np.amax(E), 3) + 0.15*np.amax(E)))
#     ax5.set_xticklabels('')
#
#     ax6.plot(x_grid, F, 'g',label = 'F')
#     ax6.set( xlabel = 'space (mm)', ylabel = '[F]', ylim=(0, round(np.amax(F), 3) + 0.15*np.amax(F)))
#
...
def plot_redgreen_contrast(final_concentration, mm, mechanism, shape, filename, path, dimension='1D', scale_factor=10, save_figure=False):
    ...
    if save_figure == True:
        plt.savefig(path + '/6eq/numerical_confocal/results/figures/1M_growingcolony/%s_%s.jpeg' % (dimension, filename))
        plt.close()
    else:
        plt.show()
...
def show_rgbvideo(timeseries_unstacked):
    ...
    for time in range(len(rgb_timeseries[:150])):
