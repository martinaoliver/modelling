...
circuit_n=10
variant=7 #variant1 is variant0 but with varying kce as well.
...
def parameterfile_creator_function(numbercombinations):
    ...
    alphaH_distribution = [x for x in loguniformdist if alphaH_range[0] <= x <= alphaH_range[1]]
    R_distribution = [x for x in loguniformdist if R_range[0] <= x <= R_range[1]]
    ...
    lenghtsdistributions = ( len(Vm_distribution), len(km_distribution), len(mu_distribution),len(alphaH_distribution),len(R_distribution),len(d_B_distribution))
    ...
    mu_distribution = mu_distribution[:minimumlenghtdistribution]
    alphaH_distribution = alphaH_distribution[:minimumlenghtdistribution]
    R_distribution = R_distribution[:minimumlenghtdistribution]
    ...
    Vm_matrix = np.column_stack(
        (Vm_distribution, Vm_distribution, Vm_distribution, Vm_distribution))
    ...
    alphaH_matrix = np.column_stack( (alphaH_distribution)).transpose()
    R_matrix = np.column_stack( (R_distribution)).transpose()
    ...
    par_distribution_matrix = np.concatenate(( Vm_matrix, km_matrix, mu_matrix,alphaH_matrix,R_matrix,d_B_matrix), 1)
    #
    ...
    kD = np.full((numbercombinations, 1), 28)
    ...
    points = np.concatenate((parameterindex, bx, bx, bx, bx, points, d_A, kD, cooperativity), 1)

    parameternames = (
    'index', 'bb', 'bd', 'be', 'bf', 'Vb', 'Vd', 'Ve', 'Vf', 'keb', 'khd', 'kfe', 'kee', 'muLVA', 'muH', 'alphaH',#'d_H':0.2,'n':0.45,'kon':2,'koff':2,'R':100
    'R', 'd_H','d_A', 'kD', 'n')
