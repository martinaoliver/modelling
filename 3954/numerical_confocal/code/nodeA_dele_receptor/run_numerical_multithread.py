...
def numerical_check(start_batch_index,n_param_sets,df, dimension,x_gridpoints,T,L,circuit_n=10, variant = 7, n_species=5,p_division=0.41,seed=1):
    save_figure = True
    ...
    for parID in df_index:
        ...
        filename = 'circuit%r_variant%s_boundary%r_%s_%sID%r_L%r_J%r_T%r_N%r'%(circuit_n,variant,boundary_coef, shape,mechanism,parID,L,J,T,N)
        ...
        if dimension == '2D':
         # Define 2D numerical parameters
            ...
            try:
                ...
                nodeA_dele_receptor_path = modelling_ephemeral + '/3954/numerical_confocal/results/figures/1M_colony_ca/2D/nodeA_dele_receptor'
                plot_redgreen_contrast(final_concentration,L,mechanism,shape,filename,nodeA_dele_receptor_path ,parID=parID,dimension=dimension,scale_factor=x_gridpoints,save_figure=save_figure)
                # rgb_timeseries = redgreen_contrast_timeseries(records)
                # show_rgbvideo(rgb_timeseries)
                if save_figure ==True:
                    pickle.dump(final_concentration, open(modelling_ephemeral + '/3954/numerical_confocal/results/simulation/1M_colony_ca/2D/nodeA_dele_receptor/2Dfinal_%s.pkl'%filename, 'wb'))
                    pickle.dump(records,open(modelling_ephemeral + '/3954/numerical_confocal/results/simulation/1M_colony_ca/2D/nodeA_dele_receptor/2Dtimeseries_%s.pkl'%filename, 'wb'))
...
df= pickle.load( open(modelling_hpc + '/3954/parameter_space_search/parameterfiles/df_circuit%r_variant%s_%rparametersets.pkl'%(10,variant,n_param_sets), "rb" ) )
