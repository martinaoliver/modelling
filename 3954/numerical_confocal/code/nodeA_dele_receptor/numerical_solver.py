...
def adi_ca(par_dict, initial_condition, L_x, L_y, J, I, T, N, circuit_n, boundary_coef=1, perturbation=0.001,timeresolution='fast',n_species = 6,tqdm_disable=True,p_division=0.4,seed=1):
    np.random.seed(seed)

    # parent_list = [circuit1_eq, circuit2_eq,circuit3_eq,circuit4_eq,circuit5_eq,circuit6_eq,circuit7_eq,circuit8_eq,circuit9_eq]
    eq = circuit10_eq(par_dict)
    d_H = eq.d_H
    diffusion_rate_list = [0,d_H,0,0,0]
    ...
    B_new,H_new,D_new,E_new,F_new  = X_new_list
    ...
    for n in tqdm(range(N),disable = tqdm_disable):
        ...
        f_curr_B,f_curr_H,f_curr_D,f_curr_E,f_curr_F = [eq.function_list(species_list,wvn=0)[i] for i in range(n_species)]
#         f_curr_B = eq.dBdt_f(species_list)
#         print('daefgrsehtdjyfyhgfE`ratseyru')
#         print(f_curr_B)
#         f_curr_H = eq.dHdt_f(species_list)
...
# #
        ...
        X_new_list = [B_new,H_new,D_new,E_new,F_new]
