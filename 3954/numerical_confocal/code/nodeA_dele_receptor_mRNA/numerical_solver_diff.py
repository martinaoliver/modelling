...
def adi_ca(par_dict, initial_condition, L_x, L_y, J, I, T, N, circuit_n, boundary_coef=1, perturbation=0.001,timeresolution='fast',n_species = 9,tqdm_disable=True,p_division=0.41,seed=1):
    ...
    for species_index in range(n_species):
        X_new_list.append(np.zeros([J, I])) #single time frame
        if timeresolution != 'slow':
            X_record_list.append(np.zeros([J, I, T])) #all time frames
    ...
    for n in tqdm(range(N),disable = tqdm_disable):
        ...
        f_curr_mB,f_curr_H,f_curr_mD,f_curr_mE,f_curr_mF,f_curr_B,f_curr_D,f_curr_E,f_curr_F = [eq.function_list(species_list,cell_grid,wvn=0)[i] for i in range(n_species)]
        print('`aszgdh')
        ...
        for i in range(I):
            ...
            H_new[i, :] = (solve_banded((1, 1), matrix_A_list[1], b_X(1, i) + f_curr_H[i, :] * dt / 2))
            print(i)
            print(matrix_A_list[1])
            print( b_X(1, i))
            print()
        ...
        for j in range(J):
            ...
            H_new[:, j] = (solve_banded((1, 1), matrix_C_list[1], d_X(1, j) + f_curr_H[:, j] * dt / 2))
            # print('dif')
            # print(f_curr_H[i,:])
            # print('dif')
            # print(H_new)
