...
def numerical_check(start_batch_index,n_param_sets,df,x_gridpoints, t_gridpoints,T,L,circuit_n=8, variant = 0, n_species=3,p_division=0.5,seed=1):
    save_figure = True
    ...
    for parID in df_index:
     ...
     # Define 2D numerical parameters
        ...
        try:
            ...
            # Run 2D simulation
            U_record,U_final = adi_ca(par_dict,L_x,L_y,J,I,T,N, circuit_n,n_species, D,tqdm_disable=tqdm_disable, stochasticity=1)#,p_division=p_division,seed=seed)
            savefig_path = modelling_ephemeral + '/3954/numerical_confocal/results/figures/1M_colony_ca/2D/nodeA_dele_stochastic'
            ...
            if save_figure ==True:
                pickle.dump(U_final, open(modelling_ephemeral + '/3954/numerical_confocal/results/simulation/1M_colony_ca/2D/nodeA_dele_stochastic/2Dfinal_%s.pkl'%filename, 'wb'))
                pickle.dump(U_record,open(modelling_ephemeral + '/3954/numerical_confocal/results/simulation/1M_colony_ca/2D/nodeA_dele_stochastic/2Dtimeseries_%s.pkl'%filename, 'wb'))
