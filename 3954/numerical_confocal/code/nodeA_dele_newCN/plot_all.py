...
parID_list = pickle.load( open(modelling_ephemeral + '/3954/numerical_confocal/results/simulation/1M_colony_ca/2D/nodeA_dele_stochastic/parID_list_variant0_10x15T120N1200.pkl', "rb" ) )
...
L,T,J,N = [10,120,150,1200]
dimension='2D'
x_gridpoints=15
details = 'modifiednormalisation'
# k=20
num=len(parID_list)
print(num)
...
for count,n in tqdm(enumerate(parID_list), disable=True):
    ...
    final_concentration = pickle.load( open(modelling_ephemeral + '/3954/numerical_confocal/results/simulation/1M_colony_ca/2D/nodeA_dele_stochastic/2Dfinal_%s.pkl'%filename, 'rb' ) )
    ...
    # if np.amin(final_concentration)<0:
    #     ax.set_ylabel(str(parID) + '-' ,size= 1,c='y', labelpad=0.35)
    # else:
    ax.set_ylabel(parID , size= 1,c='y', labelpad=0.35)


# plt.title('1M numerical search 0-%r'%num)
plt.savefig(modelling_home + '/3954/numerical_confocal/results/figures/redgreen/large_images/1M_numerical_search_%s-%s_%s_%s.png'%(start,stop,mechanism,details), dpi=2000)
# plt.show()
