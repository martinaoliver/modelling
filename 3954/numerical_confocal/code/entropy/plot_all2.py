...
parID_dict = pickle.load( open('entropy_ordered_dict_fulldataset.pkl', "rb" ) )
parID_list = []
entropy_list = []
for item in parID_dict:
    parID_list.append(item[0])
    entropy_list.append(item[1][2]/np.abs(item[1][0] - item[1][1]))
for i in zip(parID_dict,entropy_list):
    print(i)
...
#     print(row) # row in Z for elements of cluster iqstat
...
for count,n in enumerate(parID_list):
    ...
    final_concentration = pickle.load( open(modelling_ephemeral + '/3954/numerical_confocal/results/simulation/1M_colony_ca/2D/full_circuit/2Dfinal_%s.pkl'%filename, 'rb' ) )
    ...
    entropy = entropy_list[count]
    ax.set_ylabel(str(parID) + ' - ' + str(round(entropy, 2)),size= 1,c='y', labelpad=0.35)


# plt.title('1M numerical search 0-%r'%num)
plt.savefig(modelling_home + '/3954/numerical_confocal/results/figures/redgreen/large_images/1M_numerical_search_%s_%s-%s_%s.png'%(variant,start,stop,details), dpi=2000)
# plt.show()
