...
for count,parID in enumerate(striped_parID):
    ...
    plt.imshow(timeseries_unstacked[149].astype('uint8'), origin= 'lower')
    plt.axis('off')
#    plt.set_title(str(parID),fontsize=5)
# fig.subplots_adjust(hspace=-0.6,wspace = -0.1)
    plt.tight_layout()
    plt.savefig('peakresults/figures/stripped/turingstripped_parID%r'%parID)
    plt.close()
    plt.clf()
...
for count,parID in enumerate(nonstriped_parID):
    filename = 'redgreen_ADI_circuit2boundary1_growing_colony_turingID%r_L8_J160_T300_N241800.pkl'%parID
    timeseries_unstacked = pickle.load( open( ephemeral_numerical + '/results/simulation/redgreen/8x20_T300/%s'%filename, 'rb' ) )
    # plt=plt.subplot(n_row,n_cosl, count+1)
    plt.imshow(timeseries_unstacked[149].astype('uint8'), origin= 'lower')
    plt.axis('off')
 #   plt.set_title(str(parID),fontsize=5)
# fig.subplots_adjust(hspace=-0.6,wspace = -0.1)
    plt.tight_layout()
    plt.savefig('peakresults/figures/nonstripped/turingnonstripped_parID%r'%parID)
    plt.close()
