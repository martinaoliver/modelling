...
file = open(ephemeral_numerical_results + '/simulation/redgreen/nonturing/8x8_T150/parID_list.txt')
...
def count_peaks(timeseries_unstacked):
    ...
    red_diameter,green_diameter = (timeseries_unstacked[-1][int(64/2),:,i] for i in range(2))
...
for parID in parID_list:
    ...
    timeseries_unstacked = pickle.load( open(ephemeral_numerical_results + '/simulation/redgreen/nonturing/8x8_T150/%s'%filename, 'rb' ) )
    red_diameter,green_diameter = (timeseries_unstacked[-1][int(64/2),:,i] for i in range(2))
...
pickle.dump( striped_parID, open( "peakresults/striped_parID_nonturingfulldataset.pkl", "wb" ) )
pickle.dump( nonstriped_parID, open( "peakresults/nonstriped_parID_nonturingfulldataset.pkl", "wb" ) )
