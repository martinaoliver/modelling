...
def numerical_check(start_batch_index,n_param_sets,df, dimension,x_gridpoints,T,L,circuit_n=8, variant = '5716gaussian', n_species=4,p_division=0.41,seed=1):
    save_figure = True
        ...
        if dimension == '2D':
         # Define 2D numerical parameters
            ...
            try:
                ...
                nodeA_dele_path = modelling_ephemeral + '/3954/numerical_confocal/results/figures/1M_colony_ca/2D/nodeA_dele'
                plot_redgreen_contrast(final_concentration,L,mechanism,shape,filename,nodeA_dele_path ,parID=parID,dimension=dimension,scale_factor=x_gridpoints,save_figure=save_figure)
                # rgb_timeseries = redgreen_contrast_timeseries(records)
                # show_rgbvideo(rgb_timeseries)
                if save_figure ==True:
                    pickle.dump(final_concentration, open(modelling_ephemeral + '/3954/numerical_confocal/results/simulation/1M_colony_ca/2D/nodeA_dele/2Dfinal_%s.pkl'%filename, 'wb'))
                    pickle.dump(records,open(modelling_ephemeral + '/3954/numerical_confocal/results/simulation/1M_colony_ca/2D/nodeA_dele/2Dtimeseries_%s.pkl'%filename, 'wb'))
