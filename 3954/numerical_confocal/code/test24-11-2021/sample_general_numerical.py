...
for parID in interesting_list:
    ...
    mechanism = 'general'#'ATC_bullseye'
    ...
    filename = 'circuit%r_variant%s_boundary%r_%s_%sID%r_L%r_J%r_T%r_N%r'%(circuit_n,variant,boundary_coef, shape,mechanism,parID,L,J,T,N)#,p_division,kce)
    print(filename)
    if dimension == '1D':
        try:
    ...
    if dimension == '2D':
     # Define 2D numerical parameters
        ...
        try:
            ...
            records,final_concentration,grids = adi(par_dict,initial_condition,L_x,L_y,J,I,T,N, circuit_n, boundary_coef=boundary_coef,tqdm_disable=tqdm_disable,n_species=n_species)#,p_division=p_division,seed=seed)
            ...
            plot_redgreen_contrast(final_concentration,L,mechanism,shape,filename,savefig_path,parID=parID,dimension=dimension,scale_factor=x_gridpoints,save_figure=save_figure)
