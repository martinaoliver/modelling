...
def ca(par_dict, initial_condition, L_x, L_y, J, I, T, N, circuit_n, boundary_coef=1, perturbation=0.001,timeresolution='fast',n_species = 6,tqdm_disable=True,p_division=0.41,seed=1):
    np.random.seed(seed)
...
for parID in interesting_list:
    ...
    if dimension == '2D':
     # Define 2D numerical parameters
        ...
        print(np.shape(cell_grid_list[0][0]))
        # print(np.shape(cell_grid_list(int(80/2),:,0)))
        lenght_list = []
        for i in range(len(cell_grid_list)):
