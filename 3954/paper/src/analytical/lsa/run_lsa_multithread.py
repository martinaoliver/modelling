...
def lsa_check(start_batch_index,n_samples,df,circuit_n=circuit_n, variant=variant, n_species=n_species):
    ...
    pickle.dump(output_df, open(modellingpath + '/3954/paper/out/analytical/lsa_dataframes/all_dataframes/lsa_df_%s_variant%s_%rparametersets_balancedSemiBalanced_batch%r.pkl'%(circuit_n,variant,n_samples, start_batch_index), 'wb'))
...
df= pickle.load( open(modellingpath + "/3954/paper/input/fitted_parameterfiles/df_%s_variant%s_%rparametersets_balancedSemiBalanced.pkl"%(circuit_n,variant,n_samples), "rb"))
print(df)
print('df_loaded')

df = df.iloc[start_parameter:start_parameter+n_analysed_param_sets]

# print('df_%s_variant%s_%rparametersets_balanced.pkl'%(circuit_n,variant,n_samples))
...
for start_batch_index in batch_indices:
    try:
        my_data[start_batch_index] = pickle.load(open(modellingpath + '/3954/paper/out/analytical/lsa_dataframes/all_dataframes/lsa_df_%s_variant%s_%rparametersets_balancedSemiBalanced_batch%r.pkl'%(circuit_n,variant,n_samples,start_batch_index), "rb" ) )
