        ...
        for x_pos in np.linspace(1,len(cell_matrix)-2,len(cell_matrix)-2):
            ...
            if cell_matrix[y_pos, x_pos]!=0: #if there is a cell
                ...
                if cell_matrix_fastGrowth[y_pos, x_pos]==1: #if cell is fast growing
                    p_division_recalculated = p_division*2 #fast growing cells divide twice as fast
                else:
...
def maskFunction_fastMother(L=9, dx=0.05, T=50, dt=0.05, divisionTimeHours=1, p_division=0.5,seed=1,plot1D=False, plotScatter = False, plotVideo=False):
...
# cell_matrix_record,memory_matrix_record, daughterToMotherDictList = maskFunction_fastMother(L=20,dx=0.1, T=100 ,dt=0.5, divisionTimeHours=0.5, p_division=0.4, plot1D=True, plotScatter=True)
