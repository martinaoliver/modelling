...
def cell_automata_colony(cell_matrix, p_division):
    ...
    for y_pos in np.linspace(1,len(cell_matrix)-2,len(cell_matrix)-2):
        for x_pos in np.linspace(1,len(cell_matrix)-2,len(cell_matrix)-2):
            # if y_pos > len(cell_matrix)/2 and x_pos > len(cell_matrix)/2:
            if y_pos > len(cell_matrix)/2:
                p_division_specific = p_division[0]
            else:
                p_division_specific = p_division[1]
            y_pos = int(y_pos)
            x_pos = int(x_pos)
            if cell_matrix[y_pos, x_pos]!=0:
                ...
                if 0 in neighbours_cellmatrix:
                    cell_division=np.random.choice([1,0],p=[p_division_specific,1-p_division_specific])
                    if cell_division==1:
...
# numba.jit(nopython=True)
def adi_ca(L,dx,J,T,dt,N,n_species,divisionTimeHours,tqdm_disable=False,p_division=[0.01,0.1],stochasticity=0, seed=1, growth='Fast'):
    I=J
    print(f'dt{dt}')
...
def maskFunction_twogrowthrates(L=9, dx=0.05, T=50, dt=0.05, divisionTimeHours=1, p_division=[0.01,0.1],seed=1,plot1D=False, plotScatter = False, plotVideo=False):
...
#%%
# cell_matrix_record,memory_matrix_record, daughterToMotherDictList = maskFunction_twogrowthrates(L=20,dx=0.1, T=60 ,dt=0.5, divisionTimeHours=0.5, p_division=[1,0.5], plot1D=True, plotScatter=True, plotVideo=False)

# cell_matrix_record,memory_matrix_record, daughterToMotherDictList = maskFunction(L=20,dx=0.5, T=50 ,dt=0.5, divisionTimeHours=0.5, p_division=0.15, plot1D=True, plotScatter=True)
