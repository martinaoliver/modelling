...
df= pickle.load( open(modellingpath + '/3954/paper/input/gaussian_parameterfiles/df_circuit%r_variant%s_%rparametersets.pkl'%(circuit_n,variant,nsamples), "rb" ) )
...
try:
    ...
    cell_matrix_record, memory_matrix_record, daughterToMotherDictList = maskFunction_twoColonies(L=L,dx=dx, T=T, dt=dt, division_time_hours=division_time_hours, p_division=p_division, plot1D=True, plotScatter=True)
