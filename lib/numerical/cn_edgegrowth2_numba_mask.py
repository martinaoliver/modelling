...
def linear_growth(t,rate, initialL=dx):
    
    return initialL + t*rate

# for ti in tqdm(range(N), disable = tqdm_disable): 
mask = np.zeros([int(N/10), J])
for t in range(int(T/10)):
    ...
    print(t)
    mask[t] = linear_growth(t,rate, initialL=dx)
    print(linear_growth(t,rate, initialL=dx))
plt.imshow(mask)
plt.show()
...
def create_growth_mask(shape, rate , total_time):
    height, width = shape
    middle_point = width // 2
    mask = np.zeros((height, width))

    for t in range(total_time):
        growth_extent = int(1 + t*250/200)

        start = max(middle_point - growth_extent, 0)
        end = min(middle_point + growth_extent, width)
        print(growth_extent,start,end)
        mask[t, start:end] = 1
        
    return mask

# Example usage
growth_rate = 1  # Set the growth rate per time step
total_time = 200  # Total time steps
mask = create_growth_mask((200, 500), growth_rate, total_time)
plt.imshow(mask)
plt.show()